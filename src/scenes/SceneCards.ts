...
const CARD_TYPES = ['02', '03', '04', '05', '06', '07', '08', '09', '10', 'jack', 'queen', 'king', 'ace']
...
export default class SceneCards extends GameScene {
    ...
    constructor(gameController: GameController) {
        ...
        this.background = this.addChild(new Sprite('./assets/images/cards_background.jpg'))
        this.deckStartContainer = this.addChild(new Sprite())
        this.maskCardStart = this.deckStartContainer.addChild(new Sprite('card_empty'))
        this.deckFinishContainer = this.addChild(new Sprite())
        this.maskCardFinish = this.deckFinishContainer.addChild(new Sprite('card_empty'))
        ...
        for(let i = 0; i < cardsQuantity; i++) {
            ...
            const card = this.deckStartContainer.addChild(new Sprite(suit + '_' + type))
            const cardPos = this.getPositionInDeck(this.deckStartContainer)
            card.position.set(cardPos.x, cardPos.y)
            card.visible = false
        }

        this.deckStartContainer.children[this.deckStartContainer.children.length - 1].visible = true
    }
    ...
    animateCard() {
        ...
        const currentPositionInLocalCoords = this.deckFinishContainer.toLocal(card, this.deckStartContainer)
        this.deckFinishContainer.addChild(card)
        card.position.set(currentPositionInLocalCoords.x, currentPositionInLocalCoords.y)
        ...
        return gsap.to(card.position, {
            ...
            ease: 'power2.out',           
            onStart: () => {
                const maskCard = this.deckStartContainer.children[cards.length - 1]
